CREATE VIEW `swofr-billing-prod.consolidado_src.consumos_por_account`
...
FROM ( SELECT *, (total_gcp*0.9 + total_gmp*0.8) AS cargo_google,
  ...
  FROM ( SELECT costes.billing_account_id, account.Billing_Model__c AS billing_model,
    ...
    FROM `swofr-billing-prod.billing_views.sum_costs_credits_per_month` costes
    LEFT JOIN `swofr-billing-prod.billing_views.billing_accounts` account ON costes.billing_account_id = account.billing_account_id__c ));
