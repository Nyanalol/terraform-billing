SELECT
  ...
  IFNULL( SUM(IF( NOT (is_third_party OR is_third_party_marketplace OR (project_id IS NULL AND reseller_margin_gcp=0) ), cost_gcp,0)),0) AS cost_gcp,
  IFNULL(SUM(IF( NOT (is_third_party OR is_third_party_marketplace OR (project_id IS NULL AND reseller_margin_gcp=0) ), credits_gcp,0)),0) AS credits_gcp,
  project_id,
  IFNULL(SUM(reseller_margin_gmp),0) AS reseller_margin_gmp,
  IFNULL(SUM(reseller_margin_gcp),0) AS reseller_margin_gcp,
  currency,
  account,
  SUM(IF( is_third_party OR (project_id IS NULL AND reseller_margin_gcp=0 AND NOT is_third_party_marketplace)  , IFNULL(customer_cost,0),0)) AS cost_thirdparty,
  SUM(IF( is_third_party OR (project_id IS NULL AND reseller_margin_gcp=0 AND NOT is_third_party_marketplace)  , IFNULL(cost_gcp,0),0)) AS cost_base_thirdparty,
  SUM(IF( is_third_party OR (project_id IS NULL AND reseller_margin_gcp=0 AND NOT is_third_party_marketplace)  , IFNULL(credits_gcp,0),0)) AS credits_thirdparty
FROM 
  (
  SELECT
  ...
  WHERE 1=1
    ...
    AND NOT (service.description IN ( 'Distance Matrix API',
        'Street View API',
