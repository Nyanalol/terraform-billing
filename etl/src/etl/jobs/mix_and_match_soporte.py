"""
Mix and Match - Soporte Pipeline

Materializes support technical lines for accounts with Desglosar_Facturas='SI'
and loads to SF Carga_de_lectura__c + BQ output table.

Usage:
    python -m src.etl.jobs.mix_and_match_soporte --country win_uk --month 05 --year 2026
"""
...
# Derived from Talend tMap_11 → tSalesforceOutput_5
# ───────────────────────────────────────────────────────────────────────────


def _map_soporte_to_sf(
...
async def run_mix_and_match_soporte(config: MixAndMatchConfig) -> dict[str, Any]:
    """
    Execute mix_and_match soporte pipeline:
      1. Materialize: Execute materialize_importes_soporte.sql → soporte_new
      2. SF insert: soporte_new → Carga_de_lectura__c
      3. BQ write: soporte_new → bq_output_table_flex_desglosadas (DELETE+INSERT)

    Requires staging tables (stg_opportunities, stg_line_items) to exist.
    Run mix_and_match_staging first.
    """
    logger.info(
        "mix_and_match_soporte_start",
    ...
    try:
        # Phase 1: Materialize
        execute_materialize_query(config, loader, "materialize_importes_soporte")

        # Phase 2: Read materialized data
        fq_temp = (
            f"`{config.gcp_project_id}"
            f".{config.bq_transformed_dataset}.soporte_new`"
        )
        data = loader.query_to_dicts(f"SELECT * FROM {fq_temp}")
        stats["soporte_rows"] = len(data)
        logger.info("soporte_rows_materialized", count=len(data))

        if not data:
            logger.warning("no_soporte_data_materialized")
            return stats

        # Phase 3: SF insert
        sf_records = [_map_soporte_to_sf(row, config) for row in data]
    ...
    logger.info("mix_and_match_soporte_complete", stats=stats)
    return stats
...
async def main(country: str, month: str, year: str) -> int:
    ...
    config = MixAndMatchConfig(
        ...
        bq_currencies_project=settings.bq_currencies_project,
        bq_currencies_dataset=settings.bq_currencies_dataset,
        bq_currencies_table=settings.bq_currencies_table,
    ...
    try:
        await run_mix_and_match_soporte(config)
        return 0
    except Exception as e:
        logger.error("mix_and_match_soporte_failed", error=str(e), exc_info=True)
        return 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Mix and Match - Soporte Pipeline"
    )
